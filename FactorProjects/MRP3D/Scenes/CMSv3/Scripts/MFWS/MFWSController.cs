...
namespace FactorProjects.MRP3D.Scenes.CMSv3.Scripts
{
    public class MFWSController : ItemHolder, Resetable, LinkedToPlane, IHasStatus<MFWSStatus>
    {
        ...
        public void DecideAndStartProcessItem(int todoPid)
        {
            ...
            ProcessRecord p = _planeController.ProcessList[todoPid];
            if (p == PConsts.NullProcessRecord)
            {
                Hold();
                return;
            }
        ...
        IEnumerator ProcessItemToOutput((Item,ProcessRecord) ip)
        {
            var (item, process) = ip;
            Assert.IsNotNull(item);
            Assert.AreEqual(item.itemType,process.inputType);
            Assert.AreNotEqual(process,PConsts.NullProcessRecord);
