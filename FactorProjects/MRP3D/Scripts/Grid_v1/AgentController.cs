    ...
    public class AgentController : Agent
{
    ...
    void FixedUpdate()
    {
        ...
        {
            carriage.transform.position = _robot.transform.position + Vector3.up*.5f*count++;
        }
    }
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        Vector3 movement = _robot.transform.forward * act[0];
        Vector3 rotation = _robot.transform.up * act[1];
        _rigidbody.AddForce(movement * moveSpeed * (1-_rigidbody.velocity.magnitude/maxSpeed), ForceMode.VelocityChange);
        _robot.transform.Rotate(rotation*rotateSpeed);
    }
    
    public override void CollectObservations(VectorSensor sensor)
    {
        sensor.AddObservation(carriages.Count);
        sensor.AddObservation(_bracket.transform.position-_robot.transform.position);
    }
